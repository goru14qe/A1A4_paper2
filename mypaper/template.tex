...
\begin{figure}[h]
\centering
\subfloat[Pos\,1]{\includegraphics[width=0.48\linewidth]{30_Re_100/surface/pos1_Surface_plot.png}}\hfill
\subfloat[Pos\,3]{\includegraphics[width=0.48\linewidth]{30_Re_100/surface/pos3_Surface_plot.png}}
...
(L13-L17) of each topology, Reynolds number and measurement position; reproducing all of
these contour sets here would be neither practical nor informative, as the layers are
mutually similar by virtue of the flow locking established in Sec.~\ref{sec:locking}. We
therefore present a single representative case--the $30^\circ$ packing at $Re_P = 300$,
Pos\,3, which shows the closest agreement of the dataset ($R^2 = 0.93$, $\mathrm{NRMSE} =
5.1\%$; Table~\ref{tab:validation})--and note that it is typical of the regular-packing
comparisons.

Figure~\ref{fig:validation_layers} compares the simulated and measured fields in the top
three layers. The velocity-magnitude contours and in-plane streamlines
(Fig.~\ref{fig:validation_layers}a) reproduce the measured jet-recirculation structure of
each layer, and the absolute-difference maps remain small and unbiased across the plane,
the residuals concentrating in thin shear layers at the jet edges where the steepest
gradients amplify any sub-pixel misregistration between simulation and PIV. The
quantitative line profiles (Fig.~\ref{fig:validation_layers}b), taken at four streamwise
heights within the $10\,\mathrm{mm}$ layer, show the simulated profiles of layers L15, L16
and L17 collapsing onto one another and lying within the experimental $\pm\sigma$ band over
almost the entire span. This figure therefore makes two points at once: the simulation
faithfully reproduces the PIV-measured layer flow, and the three layers are mutually
interchangeable--a direct, measurement-anchored manifestation of the flow locking
quantified in Sec.~\ref{sec:locking}.

\begin{figure}[h]
\centering
\subfloat[exp.\ (left) vs.\ sim.\ (right) velocity-magnitude contours and streamlines]{%
\includegraphics[width=0.92\linewidth]{30_Re_300/layers/Combined_Exp_Sim_Pos3_Re300.png}}\\
\subfloat[velocity-magnitude profiles at four heights: sim.\ layers L15-L17 vs.\ PIV mean $\pm\sigma$]{%
\includegraphics[width=0.92\linewidth]{30_Re_300/layers/Line_Profiles_from_Bottom_Pos3_Re300.png}}
\caption{Representative layer-resolved validation for the $30^\circ$ packing at
$Re_P = 300$, Pos\,3 (layers L15-L17). (a)~Time-mean velocity-magnitude contours with
in-plane streamlines, experiment (left) versus simulation (right); (b)~velocity-magnitude
profiles at $2$, $4$, $6$ and $8\,\mathrm{mm}$ from the lower layer face, comparing the
three simulated layers (coloured) with the PIV mean and its $\pm\sigma$ band (blue). The
simulated layers reproduce the measured flow and collapse onto one another, jointly
confirming validation accuracy and flow locking. Full per-layer contour sets for all
cases are provided as supplementary material.}
